...
class RIARXPSO(metaclass = str_meta):   
    ...
    def m_function(self):        
        ...
        A = -.5 + rand                        
        M = np.eye(self.D) + 3*np.pi/180 * (A - A.conj().T)
        return M
    ...
    def getVelocity(self,p,i):
        if self.dir == -1:
            ...
            M2 = self.m_function()
            M3 = self.m_function()
            aa = self.c1[p] * npr.uniform() * (M1 @ (self.pbest[p] - self.X[p]).conj().T).conj().T +\
                 self.c2[p] * npr.uniform() * (M2 @ (self.pbest[self.neighbor[p][self.tmpid]] * (1+self.pmiu2[p] * npr.randn(self.D)) - self.X[p]).conj().T).conj().T +\
                 self.c3[p] * npr.uniform() * (M3 @ (self.gbestrep[p] * (1+self.pmiu[p] * npr.randn(self.D)) - self.X[p]).conj().T).conj().T                        
            aa = -1 * aa
        else :
            aa = self.c1[p] * npr.uniform() * (self.pbest[p] - self.X[p]) +\
                 self.c2[p] * npr.uniform() * (self.pbest[self.neighbor[p][self.tmpid]] - self.X[p]) +\
                 self.c3[p] * npr.uniform() * (self.gbestrep[p] - self.X[p])        
        #FIM_IF
